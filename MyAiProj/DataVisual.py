...
class DataVisualizer:
    def __init__(self, db_path):
        ...
        self.param_keys = ['Formula', 'Concentration', 'Temperature', 'SoakTime', 'PH']
        self.target_keys = ['Coverage', 'Uniformity', 'Adhesion']  # 目标参数
        
    def read_data(self, table_name='BayesExperData', proj_name=None, iter_id=None):
        """从SQLite数据库读取数据，支持按ProjName和IterId筛选"""
    ...
    def plot_color_mapping(self, color_by, figsize=(15, 6), save_path=None, proj_name=None, iter_id=None):
        ...
        for j in range(ys.shape[0]):
            ...
            patch = PathPatch(
                path, facecolor='none', lw=3, alpha=0.6, edgecolor=colors[colo[j]]  # 线条加粗
            )
    ...
    def _display_exp_parameters(self):
        """显示每个ExpID对应的参数信息"""
        print("\nExpID Parameter Information:")
        print("="*80)
        print(f"{'ExpID':<8}{'Formula':<10}{'Concentration':<15}{'Temperature':<12}{'SoakTime':<10}{'PH':<6}")
        print("-"*80)
        
        exp_ids = self.data['ExpID'].unique()
        for exp_id in exp_ids:
            exp_data = self.data[self.data['ExpID'] == exp_id].iloc[0]
            print(f"{exp_id:<8}{exp_data['Formula']:<10}{exp_data['Concentration']:<15}{exp_data['Temperature']:<12}{exp_data['SoakTime']:<10}{exp_data['PH']:<6.2f}")
        print("="*80)
    ...
    def plot_all_iteration_curve(self, visualization_dir=None):
        ...
        self.plot_iteration_curve(
            save_path=os.path.join(visualization_dir, f"{proj_name}_{iter_id}_IterValue.png"),
            deg=2, 
            smooth=80
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--db_path', type=str, default="D:\Parameter\Meta\DB\sampleData.db", 
    ...
    try:
        # 创建可视化器实例
        visualizer = DataVisualizer(args.db_path)
        
        # 读取数据
        print(f"【INFO】Reading data for Project: {args.proj_name}, Iteration: {args.iter_num}")
        visualizer.read_data(proj_name=args.proj_name, iter_id=args.iter_num)
